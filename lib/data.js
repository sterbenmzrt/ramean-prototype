...
// Grup yang diikuti user (area akun "Grup Saya").
export async function getMyGroups(userId) {
  ...
  return subs.map((s) => ({
    subscriptionId: s.id,
    status: s.status,
    nextBillingDate: s.nextBillingDate ?? s.group.renewalDate,
    groupId: s.group.id,
    groupCode: s.group.id.slice(-4).toUpperCase(),
    groupStatus: s.group.status,
    pricePerSlot: s.group.pricePerSlot,
    filledSlots: s.group.filledSlots,
    totalSlots: s.group.totalSlots,
    serviceName: s.group.service.name,
    logoUrl: s.group.service.logoUrl,
  }));
}

// Riwayat transaksi user (area akun "Riwayat").
export async function getMyTransactions(userId) {
  ...
  return txns.map((t) => ({
    ...
    label: t.subscription
      ? `${t.subscription.group.service.name} — Grup ${t.subscription.group.id
          .slice(-4)
          .toUpperCase()}`
      : t.type === "TOPUP"
      ? "Top-up Saldo"
...
// Banner promo aktif untuk carousel beranda (urut tampil).
